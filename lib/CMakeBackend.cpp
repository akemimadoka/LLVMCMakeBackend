...
function(_LLVM_CMAKE_BYTEARRAY_TO_STRING _LLVM_CMAKE_BYTEARRAY_TO_STRING_STR_NAME)
	if(ARGC GREATER 1)
		...
		set(${_LLVM_CMAKE_BYTEARRAY_TO_STRING_STR_NAME} "")
	endif()
endfunction()
...
function(_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ _LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR _LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_RESULT)
	if(_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR MATCHES "^_LLVM_CMAKE_PTR.*\\.GEP.*:[0-9]+:(.+)$")
		set(_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR_REF_OBJ ${CMAKE_MATCH_1})
		set(${_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_RESULT} ${_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR_REF_OBJ} PARENT_SCOPE)
	elseif(_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR MATCHES "^_LLVM_CMAKE_PTR.*\\.EXT.*:(.+)$")
		set(_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR_REF_OBJ ${CMAKE_MATCH_1})
		set(${_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_RESULT} ${_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR_REF_OBJ} PARENT_SCOPE)
	elseif(_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR MATCHES "^_LLVM_CMAKE_PTR.*\\.NUL.*")
		message(FATAL_ERROR "Pointer is a null pointer")
	else()
		set(${_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_RESULT} ${_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ_PTR} PARENT_SCOPE)
	endif()
endfunction()
...
function(_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_FUNC_MODLIST _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_EXPECT_FIELD_SIZE)
	if(_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR MATCHES "^_LLVM_CMAKE_PTR.*\\.NUL.*")
		message(FATAL_ERROR "Pointer is a null pointer")
	elseif(_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR MATCHES "^_LLVM_CMAKE_PTR.*\\.GEP.*")
		message(FATAL_ERROR "Pointer is a gep pointer, which is not support now")
	else()
		_LLVM_CMAKE_EXTRACT_POINTER_REF_OBJ(${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR} _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ)
		list(LENGTH _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ_FIELD_SIZE)
		if(_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ_FIELD_SIZE LESS _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_EXPECT_FIELD_SIZE)
			math(EXPR _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_ADD_FIELD_COUNT "${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_EXPECT_FIELD_SIZE} - ${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ_FIELD_SIZE}")
			string(REPEAT ";0" ${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_ADD_FIELD_COUNT} _LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_ADD_FIELD)
			set(${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ} "${${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ}}${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_ADD_FIELD}" PARENT_SCOPE)
			list(APPEND ${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_FUNC_MODLIST} ${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_PTR_REF_OBJ})
			set(${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_FUNC_MODLIST} ${${_LLVM_CMAKE_ASSUME_MEMORY_LAYOUT_FUNC_MODLIST}} PARENT_SCOPE)
		endif()
	endif()
endfunction()
